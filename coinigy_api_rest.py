...
exchanges = namedtuple('exchanges', ('name', 'code', 'markets'))
...
class CoinigyREST:
    ...
    def all(self, exchange, market):
        return self.data(exchange=exchange, market=market, data_type='all')
...
class ExchangeLoader:
	...
	def __init__(self, coinigyInstance):
		self.coinigy = coinigyInstance
		self.allExchanges = []

	def loadExchanges(self):
		bittrex = exchanges(name='Bittrex', code='BTRX', markets=self.loadMarkets('BTRX'))
		gdax = exchanges(name='GDAX', code='GDAX', markets=self.loadMarkets('GDAX'))

		self.allExchanges = [bittrex, gdax]

	def getBTCOnly(self, pandasData):
		return pandasData[pandasData.str.contains('BTC')]

	def loadMarkets(self, exchangeCode):
		data = self.coinigy.markets(exchangeCode).loc[:, 'mkt_name']
		return data

myCredentials = credentials(api=credentials.Coinigy.API, secret=credentials.Coinigy.Secret, endpoint='https://api.coinigy.com/api/v1')

coinigy = CoinigyREST(myCredentials)
#myExchanges = ExchangeLoader(coinigy)
#myExchanges.loadExchanges()

print(coinigy.orders('GDAX', 'BTC/USD')['bids'])

# with pd.option_context('display.max_rows', None, 'display.max_columns', 3):
# 	for exchange in myExchanges.allExchanges:
# 		print(myExchanges.getBTCOnly(exchange.markets))
